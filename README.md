...
Если браузер не может выполнить скрипы, то выполняется тэг `<noscript><p>JavaScript is not enabled on your browser.</p></noscript>`
```html
...
```html
<meta name="description" content="Учбеная страница, которая помогает понять какие теги нужны и что они значат">
```
...
<q>
    ::before;
    Азотная кислота и концентрированная серная кислота с металлами взаимодействуют иначе (соль
    образуется, но водород при этом не выделяется).
    ::after
</q>
```
...
#### тэг `input`
```html
<input type="text" value="значение в поле" placeholder="текст по-умолчанию" name="имя поля">
```
`type` | описание
-- | --
...
`disabled` - отключает форму вврда
`name`
...
Доступность форсы без клавиатуры
...
%default-bg {
    background-color: rgba(#43fed2, .5);
    border-radius: 10px;
...
Инструкции джаваскипта выполняет движок v8. Используется в Chrome и NodeJs.
Механика отличается.
Инстрeкции переводятся в машинный код.
...
`type="text/javacript" language=""` - уже неактуально
...
number & number
    - NaN несравнимо ни с чем - результат всегда false. Это не число и нет вохможности сравнить его с числом. Мы не знаем что это.
...
Ещё одна важная особенность __Function Declaration__ заключается в их блочной области видимости.

В строгом режиме, когда __Function Declaration__ находится в блоке `{...}`, функция доступна везде внутри блока. Но не снаружи него.
...
function make_greetings(name) {
    ...
    const times = ["Доброй ночи", "Доброго утра", "Доброго дня", "Доброго вечера"];
...
function camelize(str) {
  return str
    ...
    .map((value, index, arr) => {      
      ...
      const ans = index > 0 ? [value[0].toUpperCase(), ...value.slice(1,)].join('') : [...value].join(''); 
      return  ans;
    })
    .join('')
}
...
const groupById = arr => arr.reduce((obj, user) => {
    const key = user.id;
    return {...obj, [key]: user};
}, {});
...
const mean = arr => parseFloat((arr.reduce((acc, val) => acc + val , 0) / arr.length).toPrecision(2));
...
```javascript
const employers = [
    {name: 'Ivan', age: 23}, {name: 'Nadya', age: 26}, {name: 'Pavel', age: 43},
];

const em_to_str = arr => arr.map( 
    empl =>  Object.keys(empl).map( (key, i) => `${key}: ${empl[key]}`).join(", ")
);
...
const is_repeated = (arr) => arr.filter(word => {
...
const filter_even = (arr) => arr.filter(num => {
    return `${num}`.split("").map(Number).reduce( (acc, num) => acc + num, 0) %2 == 0;
});
...
__Не следует использовать `map[key] = value`, т.к. в таком случае map будет рассматриваться как обычный объект, что приведет к ссотвествующим ограничениям__
...
function sets_is_eq(a, b) {
    ...
    for (let elem of a2) {
        if (b2.has(elem)) { // можно (нужно!) без проверки, сразу удалять
            a2.delete(elem);
            b2.delete(elem);
        }
        else 
            break; // несоответствие множеств
    }
    ...
    for (let ind in arr) {
        ...
        Loop: {
            for (let i = 0; i < ind; i++) {
                ...
                if (sets_is_eq(set_of_chars, prev_set_of_chars))
                    break Loop;
            }
            res.push(ind);
        }
...
function aclean(arr) {
    ...
    return arr.filter( word => {
        const ordered_set_of_chars = Array.from(word.toLocaleLowerCase()).sort().join("");
...
__Контекст выполнения__ – специальная внутренняя структура данных, которая содержит информацию о вызове функции. Она включает в себя конкретное место в коде, на котором находится интерпретатор, локальные переменные функции, значение `this` и прочую служебную информацию.
...
- __Environment Record__ - объект, в котором в свойствах храняться все значения локальных переменных, а также некотороя другая информация, например значение `this`.
...
function makeArmy() {
    ...
    for (let i = 0; i < 10; i++) {  // для каждой итерации for создается свое лексическое окружение со своей i
...
Имеет универсвльное имя __`globalThis`__.
```javascript
global.gVar = 5;
...
#### Проброс исключения
Есть простое правило: __блок `catch` должен обрабатывать только известные ему ошибки и пробрасывать все остальные.__
...
```
!__ВАЖНО__: мы можем выбирать только один метод чтения объекта. Если не уже сработал метод `response.json()` то метод `response.text()` работать не будет, т.к. данные уже были обработаны.
...
__callback__ - некая функция, которая будет выполнена по завершению асинзронной операции.
...
1. Есть __создающий код__, который делает что-то, что занимает время. Например загружает данные по сети.
2. Есть __потребляющий код__, который хочет получить то, что создает _создающий код_, когда оно будет готово. Это может быть более чем одна функция.
3. __`Promise`__ - специальный объект в JavaScript, который связывает создающий и потребляющий код вместе.
...
```
__resolve__ и __reject__ - это коллбеки, которые передаются внуть промиса.
...
Уточню, что промис может возвращать не только объект промиса, но и любой __thenable__, т.е. такой, у которого есть метод `.then`.
```javascript
class Thenable {
  constructor(value) {
    this.value = value;
...
```
В приведенном коде промис в первом `.then` получает значение `3`, а потом возвращает __thenable__ объект с методом `.then`, в который передаются методы `resolve` и `reject`.

#### __`fetch`__
Fetch запрашивает по сети `url` и возвращает промис. Когда промис успешно выполняется, он возвращает объект `response` только после того, как удаленный сервер присылает заголовки ответа, но __до того__ как будет загружено полное тело ответа.
...
Можно использовать __Promise.all__ вместе с __map__ для обработки массива ссылок, например - получать ответы и потом обрабатывать их:
```javascript
let urls = [
...
Если любой промис в `Promise.all` заверщится с ошибкой, то промис возвращенный `Promise.all` __немедленно__ завершиться с ошибкой. Все поледующий результаты промисов будут проигнорированы.
...
async function showAvatar(username) {
    const github_response = await fetch(`https://api.github.com/users/${username}`)
...
__await нельзя использовать на верхнем уровне вложенности__
...
async function f2(n) {
    const promise = new Promise( (resolve, reject) => new Thenable(3).then(resolve, reject));
    promise.then( result => {
        console.log(result);
    });
}
